...
class ConvolutionalLayer(object):
            ...
            for idxc in range(self.channel_out):
                for idxh in range(height_out):
                    for idxw in range(width_out):
                        ...
                        h_start, h_end = idxh * self.stride, idxh * self.stride + self.kernel_size
                        w_start, w_end = idxw * self.stride, idxw * self.stride + self.kernel_size
                        region = self.input_pad[idxn, :, h_start:h_end, w_start:w_end]
            ...
            for idxh in range(self.height_out):
                for idxw in range(self.width_out):
                    self.img2col[idxn*self.height_out*self.width_out + idxh*self.width_out + idxw, :] = \
                        self.input_pad[idxn, :, idxh*self.stride:idxh*self.stride+self.kernel_size, idxw*self.stride:idxw*self.stride+self.kernel_size].reshape([-1])

        # 计算卷积的前向传播，特征图与卷积核的内积变为矩阵相乘，再加偏置
    ...
    def backward_speedup(self, top_diff):
        ...
        # Reshape top_diff为矩阵形式
        top_diff_reshape = top_diff.transpose([0, 2, 3, 1]).reshape(-1, self.channel_out)
        
        # 计算偏置梯度
        self.d_bias = np.sum(top_diff_reshape, axis=0)
        
        # 计算权重梯度
        self.d_weight = np.dot(self.img2col.T, top_diff_reshape).reshape(self.weight.shape)
        
        # 计算输入梯度
        d_img2col = np.dot(top_diff_reshape, self.weight_reshape.T)
        for idxn in range(self.input.shape[0]):
            for idxh in range(self.height_out):
                for idxw in range(self.width_out):
                    # 映射d_img2col回输入的梯度
                    bottom_diff[idxn, :, idxh * self.stride:idxh * self.stride + self.kernel_size, idxw * self.stride:idxw * self.stride + self.kernel_size] += \
                        d_img2col[idxn * self.height_out * self.width_out + idxh * self.width_out + idxw, :].reshape(self.channel_in, self.kernel_size, self.kernel_size)
...
class MaxPoolingLayer(object):
            ...
            for idxc in range(self.input.shape[1]):
                for idxh in range(height_out):
                    for idxw in range(width_out):
                        ...
                        h_start, h_end = idxh * self.stride, idxh * self.stride + self.kernel_size
                        w_start, w_end = idxw * self.stride, idxw * self.stride + self.kernel_size
                        region = self.input[idxn, idxc, h_start:h_end, w_start:w_end]
                        self.output[idxn, idxc, idxh, idxw] = np.max(region)
    ...
    def forward_speedup(self, input):
        ...
        start_time = time.time()
        self.input = input  # [N, C, H, W]
        self.max_index = np.zeros_like(self.input)  # 标记最大值的位置
        N, C, H, W = input.shape
        K = self.kernel_size
        S = self.stride
        ...
        # 使用 reshape 和最大值的索引来计算最大值
        self.output = np.zeros((N, C, height_out, width_out))

        for idxh in range(height_out):
            for idxw in range(width_out):
                h_start, h_end = idxh * S, idxh * S + K
                w_start, w_end = idxw * S, idxw * S + K

                # 切片区域，保持维度一致
                region = self.input[:, :, h_start:h_end, w_start:w_end]
                region_reshaped = region.reshape(N, C, -1)  # 重塑为 (N, C, K*K)

                # 取最大值及其索引
                max_vals = np.max(region_reshaped, axis=2)
                max_idxs = np.argmax(region_reshaped, axis=2)

                # 更新输出和最大值索引
                self.output[:, :, idxh, idxw] = max_vals
                max_idxs_2d = np.unravel_index(max_idxs, (K, K))  # 将线性索引转为 2D 索引

                for n in range(N):
                    for c in range(C):
                        self.max_index[n, c, h_start + max_idxs_2d[0][n, c], w_start + max_idxs_2d[1][n, c]] = 1
    ...
    def backward_speedup(self, top_diff):
        ...
        # 使用 max_index 恢复梯度
        expanded_top_diff = np.repeat(np.repeat(top_diff, self.stride, axis=2), self.stride, axis=3)
        bottom_diff = self.max_index * expanded_top_diff
