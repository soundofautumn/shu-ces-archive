...
if __name__ == '__main__':
    ...
    mlu_device = torch.device('mlu')  
    mlu_image_style = image_style.to(mlu_device)
    net = VGG19().cpu()
    g_net = TransNet().cpu()
    #TODO: 将特征网络加载到mlu得到mlu_net
    mlu_net = net.to(mlu_device)
    #TODO: 将图像转换网络加载到mlu得到mlu_g_net
    mlu_g_net = g_net.to(mlu_device)
    ...
    mlu_loss_func = loss_func.to(mlu_device)
    ...
    gram_s1 = get_gram_matrix(s1).detach()
    gram_s2 = get_gram_matrix(s2).detach()
    gram_s3 = get_gram_matrix(s3).detach()
    gram_s4 = get_gram_matrix(s4).detach()
    j = 0
    count = 0
    epochs = 100
    while j <= epochs:
        for i, image in enumerate(data_loader):
            ...
            mlu_image_c = image_c.to(mlu_device)
            ...
            loss_s1 = mlu_loss_func(gram_out1, gram_s1)
            loss_s2 = mlu_loss_func(gram_out2, gram_s2)
            loss_s3 = mlu_loss_func(gram_out3, gram_s3)
            loss_s4 = mlu_loss_func(gram_out4, gram_s4)
            ...
            if i % 10 == 0:
                ...
                concatenated = torch.cat((mlu_image_c.cpu(), mlu_image_g.cpu()), dim=3)
                save_path = f'./out/train_mlu/result_{count}.jpg'
