...
torch::Tensor hsigmoid_cpu(const torch::Tensor & dets) {
  ...
  auto input_data = dets.data_ptr<float>();
  vector<float> input_vector(input_data, input_data + dets.numel());

  int input_size = input_vector.size(); 
  //TODO: 创建一个浮点类型的output_data，output_data为大小与输入相同的vector
  vector<float> output_data(input_size, 0.0f);
  //TODO: 对于输入向量的每个元素计算hsigmoid
  for (int i = 0; i < input_size; ++i) {
    float x = input_vector[i];
    output_data[i] = 1.0f / (1.0f + exp(-x)); 
  }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {	
  m.def("hsigmoid_cpu", &hsigmoid_cpu, "H-Sigmoid Activation (CPU)");
}       
