...
class RNN_model(nn.Module):
    def __init__(self, batch_sz, vocab_len, word_embedding, embedding_dim, lstm_hidden_dim):
        ...
        # ???
        #########################################
        # 定义两层的 LSTM，输入维度为 embedding_dim，输出维度为 lstm_hidden_dim
    ...
    def forward(self, sentence, is_test=False):
        ...
        ################################################
        # 在 forward 方法中完成 LSTM 的前向传播
        # 初始化隐藏状态和细胞状态为零
        h0 = torch.zeros(2, batch_input.size(0), self.lstm_dim).to(batch_input.device)  # 两层的隐藏状态
        c0 = torch.zeros(2, batch_input.size(0), self.lstm_dim).to(batch_input.device)  # 两层的细胞状态
