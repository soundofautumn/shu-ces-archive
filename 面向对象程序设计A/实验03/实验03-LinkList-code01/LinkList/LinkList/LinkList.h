...
template<typename T>
class LinkList;    // 最外层：链表类（模板）提前声明

template<typename T>
class Node        // 中间层：结点类（模板）
{
public:
...
public:
    ...
    Node<T> *Locate(const TYPE &x, bool newsearch = false); // 当前结点依条件（给定TYPE类型）定位

    template<typename TYPE>
    void Sort(const TYPE &x, bool ascending = true);    // 根据TYPE类型排序（升序或降序）
    void Reverse();                                // 链表结点倒置
